...
function readFile()     
{
  ...
  .on('end', () => {
    ...
    .on('data', (row) => 
    {
		...
		console.log(row);
		var skip = false;
      if(row['7'] == 'Select your Program name' && row['4']=='Select your study program'){skip = true;}
      else if(row['7']!='' && row['4']=='Bachelors')
      {
        tempProgName = row['7'];
//        console.log(row['7'])
      }
	  else if(row['22']!='' && row['4']=='Masters')
      {
		  if(row['23'] != '')
			  tempMcourse = row['23'];
//        console.log(row['7'])
  ...
  //    {
	  if(!skip)
	  {
        if(allRows[count]['Select your study program']=='Bachelors')
        {
          allRows[count]['Select your Program name'] = tempProgName;
        }
		else if(allRows[count]['Select your study program']=='Masters')
        {
			allRows[count]['Select the Course name you want to review'] = tempMcourse;
		}
		count++;
	  }
   //   }
